# HITSZ CS Labs

[kuma-loong](https://github.com/kuma-loong) 的哈尔滨工业大学（深圳）计算机课程实验索引，按学期汇总个人公开的课程实验与课程设计仓库。

## 2026 年秋季

| 课程 | 仓库 | 简介 |
| --- | --- | --- |
| 操作系统 | [26Fall-OS-Lab](https://github.com/kuma-loong/26Fall-OS-Lab) | 基于 xv6 的操作系统课程实验，涵盖 Unix 实用程序、进程与系统调用、锁及页表。 |

## 2026 年夏季小学期

| 课程 | 仓库 | 简介 |
| --- | --- | --- |
| CPU 设计 | [26Summer-Soc-Design](https://github.com/kuma-loong/26Summer-Soc-Design) | 围绕 miniRV 指令集与 EGO1 开发板开展 CPU、流水线及 SoC 设计实验。 |

## 2026 年春季

| 课程 | 仓库 | 简介 |
| --- | --- | --- |
| 机器学习 | [26Spring-ML-Lab](https://github.com/kuma-loong/26Spring-ML-Lab) | 2026 年春季机器学习课程的配套实验。 |
| 数据结构与算法 | [26Spring-DSA-Lab](https://github.com/kuma-loong/26Spring-DSA-Lab) | 2026 年春季数据结构与算法课程的配套实验。 |
| 软件构造 | [AircraftWar](https://github.com/kuma-loong/AircraftWar) | 使用 Java 与 Swing 开发飞机大战游戏，实践面向对象设计、设计模式、多线程和单元测试。 |

## 2025 年秋季

| 课程 | 仓库 | 简介 |
| --- | --- | --- |
| 高性能计算应用实践 | [HPCP-fall25](https://github.com/kuma-loong/HPCP-fall25) | 涵盖 Linux 与 C 编程、进程和线程、OpenMP、MPI 及矩阵乘法性能优化。 |
